...
void show_photo_on_lcd(int fd_lcd, char photo_path[])
{
    if (strstr(photo_path, ".bmp"))
    {
        // lcd的映射--mmap
    ...
    else if (strstr(photo_path, ".jpg"))
    {
        // 处理JPEG格式图片
        lcd_draw_jpg(200, 50, photo_path, NULL, 0, 0);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    char *photo_path[] = {"1.bmp", "2.bmp", "6.jpg", "3.bmp", "7.jpg", "4.bmp", "5.bmp"};
    int photo_size = sizeof(photo_path) / sizeof(photo_path[0]);
    int current_photo = 0;

    show_photo_on_lcd(fd_lcd, photo_path[current_photo]);
    ...
    while (1)
    {
        ...
        case UP:
            printf("Slide up\n\n");
            current_photo = (current_photo - 1 + photo_size) % photo_size;
            show_photo_on_lcd(fd_lcd, photo_path[current_photo]);
            break;
        case DOWN:
            printf("Slide down\n\n");
